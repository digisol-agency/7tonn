...
if( !function_exists( 'kona_addBodyClasses' ) ) {
	function kona_addBodyClasses( $classes ) {
		...
		// ajax classes
		if (get_option('_sr_shopajaxadd') !== '0') {
			$classes[] = esc_attr('ajax-addtocart');
		}
		if (get_option('_sr_shopajaxremove') !== '0') {
			$classes[] = esc_attr('ajax-removefromcart');
		}
		if (get_option('_sr_shopajaxchange') !== '0') {
			$classes[] = esc_attr('ajax-changecart');
		}
		$classes[] = esc_attr('hello');
		return $classes;
	}
}
...
if( !function_exists( 'kona_getCategory' ) ) {
	function kona_getCategory($limit=null) {
		...
		// CATEGORY
		$metacategory = '';
		$metacategory .= '<span class="post-cat">'.esc_html__('in', 'kona').' ';
		$metacategorysimple = '';
...
if( !function_exists( 'kona_Share' ) ) {
	function kona_Share($type,$title,$align=null,$style="normal") {
		...
		if ($gplus) {
			$output .= '<li class="googleplus"><a href="" onclick="window.open(\'https://plus.google.com/share?url='.esc_url(get_the_permalink()).'&amp;image='.esc_url($og_img).'\',\'\',\'width=900, height=500, toolbar=no, status=no\'); return(false);">'.$gName.'</a></li>';
		}
		if ($pt) {
			$output .= '<li class="pinterest"><a href="" onclick="window.open(\'https://pinterest.com/pin/create/bookmarklet/?media='.esc_url($og_img).'&amp;url='.esc_url(get_the_permalink()).'\',\'\',\'width=650, height=350, toolbar=no, status=no\'); return(false);">'.$ptName.'</a></li>';
		}
		$output .= '</ul>';
