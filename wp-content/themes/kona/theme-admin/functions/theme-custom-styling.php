...
if( !function_exists( 'kona_custom_style_logo' ) ) {
	function kona_custom_style_logo() {
		...
		if ($logoDark || $logoLight) {
			...
			$fileType = wp_check_filetype($logodata[0]);
			if ($fileType['ext'] == 'svg') {
				$logoHeight = 40;
			}
			...
			body:not(.single-product) #header.has-header-bar + #hero-and-body > #page-body:first-child { margin-top:  calc('.$logoHeight.'px + 113px + 60px); }
			';
			
			/*
			body:not(.single-product) #header + #hero-and-body > #page-body:first-child { margin-top:  calc('.$logoHeight.'px + 20px + 60px); }
			body:not(.single-product) #header.has-header-bar + #hero-and-body > #page-body:first-child { margin-top:  calc('.$logoHeight.'px + 53px + 60px); }
			*/
			...
			body:not(.single-product) #header:not(.break-1024).has-header-bar:not(.hide-header-bar-mobile) + #hero-and-body > #page-body:first-child { margin-top:  calc('.$respHeight.'px + 50px + 60px) !important ; }
			body #header:not(.break-1024).menu-is-open #menu #menu-inner { padding-top: calc('.$respHeight.'px + 60px); }
			body #header:not(.break-1024).has-header-bar:not(.hide-header-bar-mobile).menu-is-open #menu #menu-inner { padding-top: calc('.$respHeight.'px + 60px + 30px); }
			body #header:not(.break-1024) #menu .scroll-menu { max-height: calc(100vh - '.($respHeight + 60).'px - 50px); }
			body #header:not(.break-1024).has-header-bar:not(.hide-header-bar-mobile) #menu .scroll-menu { max-height: calc(100vh - '.($respHeight + 60 + 30).'px - 50px); }
			...
			body:not(.single-product) #header.break-1024.has-header-bar:not(.hide-header-bar-mobile) + #hero-and-body > #page-body:first-child { margin-top:  calc('.$respHeight.'px + 50px + 60px) !important ; }
			body #header.break-1024.menu-is-open #menu #menu-inner { padding-top: calc('.$respHeight.'px + 60px); }
			body #header.break-1024.has-header-bar:not(.hide-header-bar-mobile).menu-is-open #menu #menu-inner { padding-top: calc('.$respHeight.'px + 60px + 30px); }
			body #header.break-1024 #menu .scroll-menu { max-height: calc(100vh - '.($respHeight + 60).'px - 50px); }
			body #header.break-1024.has-header-bar:not(.hide-header-bar-mobile) #menu .scroll-menu { max-height: calc(100vh - '.($respHeight + 60 + 30).'px - 50px); }
			}
			...
			body #header.has-header-bar:not(.hide-header-bar-mobile) .menu-search, body #header.has-header-bar:not(.hide-header-bar-mobile) .header-wishlist { top: calc('.$respHeaderHeight.'px + 40px + 30px + 7px); }
			}
			';
...
if( !function_exists( 'kona_custom_style_color' ) ) {
	function kona_custom_style_color() {
		...
		// Footer Color
		if (get_option('_sr_footerbg')){ 
			$return .= '#footer { background: '.get_option('_sr_footerbg').'; }';
		}
		...
		foreach($defaultfonts as $font) {
			...
			if ($font == "h3") {
				$return .= '#header-search form input[type=search], body h2.sg_title {';
					...
					if ($size) { $return .= 'font-size: '.$size.' !important; line-height: '.$lineheight.'; height: '.$lineheight.';'; }
					if ($spacing && $spacing !== '0') { $return .= 'letter-spacing: '.$spacing.'em;'; }
					if ($transform) { $return .= 'text-transform: '.$transform.';'; }
				$return .= '}';
			}
		...
		// INPUT
			...
			$return .= '@media only screen and (max-width: 768px) { 
							input[type="text"], input[type="password"], input[type="email"], input[type="number"], input[type="tel"], input[type="date"], input[type="search"], textarea, select, .select2-container .select2-selection--single .select2-selection__rendered {
								size: '.intval(intval($size)*0.85).'px !important; 
							}
						}';
		// INPUT
