    ...
    function kona_enqueue_scripts() {
		...
		wp_enqueue_script('comment-reply'); 	# pre-packaged with WordPress core
			...
			if (	
			   ...
			   ) { wp_enqueue_script('phatvideo', get_template_directory_uri() . '/files/js/jquery.min.phatvideobg.js', array('jquery'), '2.5.2', true); }
...
function kona_plugin_activation() {
	...
	$plugins = array(
		...
		array(
			'name'     				=> esc_html__('WC Ajax Product Filter', 'kona' ), // The plugin name
			'slug'     				=> 'wc-ajax-product-filter', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory_uri() . '/plugin-activation/plugins/wc-ajax-product-filter.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
