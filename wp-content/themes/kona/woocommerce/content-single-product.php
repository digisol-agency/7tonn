...
$layout = get_post_meta(get_the_ID(), '_sr_productlayout', true);
	if (!$layout && get_option('_sr_shopsingleoptionlayout') == 'modern') { $layout = "modern"; }
...
$startanimation = get_option('_sr_shopsingleoptionanim');
...
if ($layout == "modern") {
	...
	if ($showsale) { add_action( 'woocommerce_single_product_summary', 'woocommerce_show_product_sale_flash', 4 ); }
} else {
	if ($showsale) { add_action( 'woocommerce_before_single_product_summary', 'woocommerce_show_product_sale_flash', 21 ); }
...
function sr_get_prodVideo() {
	...
	if ($prodVideo) {
		if ($prodVideo == 'youtube' && get_post_meta( get_the_ID() , '_sr_productvideo_youtube', true)) {
			$href = '//www.youtube-nocookie.com/embed/'.get_post_meta( get_the_ID() , '_sr_productvideo_youtube', true).'?autoplay=1'; 
...
if (!isset($quickView)) { $colClass = $colClass.' spaced-huge'; }
	...
	if (!get_option('_sr_shopgridquickviewshare')) { 
		remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_sharing', 50 );
	}
}
