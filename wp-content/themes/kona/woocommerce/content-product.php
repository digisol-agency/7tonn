...
if ($showvariation) {
	...
	if( $product->is_type( 'variable' ) ){
		...
		if ($variations[0]['attributes'] && (count($variations[0]['attributes']) > get_option('_sr_shopgridvariationscount'))) {
	...
	<div class="shop-item-inner item-inner <?php echo esc_attr($unveil); ?> <?php echo esc_attr($itemClass); ?>">
		...
		<div class="product-media item-media">
			...
			<a href="<?php echo esc_url(get_permalink()); ?>" class="thumb-hover scale">
				<?php 
					$imgClass = ''; if ($lazy) { $imgClass = 'lazy'; }		   
						...
						foreach($variations as $v) {
							...
							if ($v['image_id'] && $v['image_id'] !== get_post_thumbnail_id( get_the_ID() )) { 
								...
								echo '<span class="variation-image" data-variation="'.$name.'">
											<img class="var-img '.esc_attr($imgClass).'" src="'.esc_url($image[0]).'" data-srcset="'.$imageSrc.'" sizes="(max-width: '.esc_attr($image[1]).'px) 100vw, '.esc_attr($image[1]).'px" width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_attr(get_the_title($v['image_id'])).'"/>
										</span>';
								$hasVarImage = true;
							}
						}
					...
					if (get_post_meta( get_the_ID() , '_sr_producthover', true) && !$hasVarImage) {
						...
						if ($imageId) { echo '<img class="hover '.esc_attr($imgClass).'" src="'.esc_url($image[0]).'" data-srcset="'.$imageSrc.'" sizes="(max-width: '.esc_attr($image[1]).'px) 100vw, '.esc_attr($image[1]).'px" width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_attr(get_the_title($imageId)).'"/>'; }
						else { echo '<img class="hover '.esc_attr($imgClass).'" src="'.esc_url(get_post_meta( get_the_ID() , '_sr_producthover', true)).'" alt="'.esc_attr(get_the_title($imageId)).'"/>'; }
						echo '</span>'; 
					}
				?>
