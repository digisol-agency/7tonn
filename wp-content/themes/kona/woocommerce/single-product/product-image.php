	...
	foreach($variations as $v) {
		...
		if ($v['sr_variation_gallery'] && $v['sr_variation_gallery'] !== ' ') {
			...
			if ($prodImage && $v['image_id']) {
				$theImage = wp_get_attachment_image_src( $v['image_id'], 'kona-thumb-big' );
				$imageAlt = get_post_meta($v['image_id'], '_wp_attachment_image_alt', true);
				if (!$imageAlt) { $imageAlt = get_the_title($v['image_id']); }
				echo '
					<div class="carousel-item">
						<div class="product-image">';
							if ($zoom) { echo '<span class="zoomF">'; }
							echo '<img src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" data-flickity-lazyload="'.esc_url($theImage[0]).'" width="'.esc_attr($theImage[1]).'" height="'.esc_attr($theImage[2]).'" alt="'.esc_html($imageAlt).'" />';
							if ($zoom) { echo '</span>'; }
				echo '
						</div>
					</div>
			...
			foreach($jsonGal->sortable as $j) {
				...
				$imageAlt = get_post_meta($j->id, '_wp_attachment_image_alt', true);
				if (!$imageAlt) { $imageAlt = get_the_title($j->id); }
				...
				<div class="product-image '.esc_attr($addClass).'">';
					if ($zoom) { echo '<span class="zoomF">'; }
					echo '<img src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" data-flickity-lazyload="'.esc_url($image[0]).'" width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_html($imageAlt).'" />';
					if ($zoom) { echo '</span>'; }
				echo '</div>';	
				echo '</div>';
			}
			...
			if ($srGallery) {
				...
				foreach ( $jsonGallery["sortable"] as $a ) {
					...
					$imageAlt = get_post_meta($a['id'], '_wp_attachment_image_alt', true);
					if (!$imageAlt) { $imageAlt = get_the_title($a['id']); }
			...
			<div class="product-image">';
				if ($zoom) { echo '<span class="zoomF">'; }
				echo '<img src="'.esc_url($image[0]).'" width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_html($imageAlt).'"  />';
				if ($zoom) { echo '</span>'; }
			echo '</div>';	
			echo '</div>';
			...
			echo '
				<div class="carousel-item">
					<div class="product-image '.esc_attr($addClass).'">';
						...
						echo ' width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_html($imageAlt).'" />';
						if ($zoom) { echo '</span>'; }
			echo '
					</div>
				</div>
