...
<div id="page-content">
	...
	<?php
		...
		if (!get_option('_sr_headerbarmiddle')) { $classHeader .= " hide-header-bar-mobile"; }
		...
		$classCart = '';
		if (get_option('_sr_shopminicartopen') && get_option('_sr_shopajaxadd') !== '0' ) { $classCart = "ajax-open"; }
		if (class_exists('Woocommerce') && get_option('_sr_shopminicartlink') && !WC()->cart->is_empty()) { $classCart .= " show-cart-link"; }
	?>
	...
	<header id="header" class="<?php echo esc_attr($classHeader); ?>">
		...
		C9,51.6,15.6,42.8,24.6,39.6z"/>
				...
				<?php } ?>
				<?php if (class_exists('Woocommerce') && get_option('_sr_headersearch')) { ?>
